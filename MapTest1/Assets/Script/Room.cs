    ...
    IPointerExitHandler
{
            ...
            else // 기타방이면
            {
                ...
                for (int i = 0; i < BeforeRoom.Length; i++) // 이전방 의존관계 설정
                {
                    ...
                    if (BeforeRoom[i].GetComponent<Room>().isSelected == true) // 진짜 이전방과 연결
                    {
                        ...
                        {
                            if (realBeforeRoom.GetComponent<Room>().AfterRoom[j] == null) break; // TODO 여기 뭔가 이상함
    ...
    IEnumerator ScaleCoroutine(Vector3 startScale, Vector3 endScale, float duration)
    {
        ...
        while (Time.time < endTime)
        {
            ...
            roomSprite.transform.localScale = Vector3.Lerp(startScale, endScale, progress); // 현재 방 스케일
            yield return null;
        }
        roomSprite.transform.localScale = endScale; // 다끝나면 1프레임 뒤에 스케일 펌핑하는거 방지
    }
